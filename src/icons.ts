/**
 * Icon shapes from Lucide (https://lucide.dev). Full license text (Lucide's own ISC
 * license, plus the underlying MIT license for icons derived from Feather Icons) is in
 * ../THIRD_PARTY_NOTICES.md. Used as CSS mask-image sources (see this file's one
 * caller, staticAssets.ts) so each icon inherits its color via the masked element's own
 * `background-color: currentColor` instead of being baked into the icon itself.
 */
const ICON_INNER: Record<string, string> = {
  ...
  todo: `<rect width="18" height="18" x="3" y="3" rx="2"/><path d="m16 9-5.5 5.5L8 12"/>`,
  ...
  generic: `<path d="M22 17a2 2 0 0 1-2 2H6.828a2 2 0 0 0-1.414.586l-2.202 2.202A.71.71 0 0 1 2 21.286V5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2z"/>`,
...
/**
 * Encodes an icon as a CSS mask-image data URI. The mask source's own stroke color is
 * irrelevant (masking only reads the rendered alpha), so it's fixed to black; the
 * masked element's `background-color: currentColor` supplies the actual visible color.
 */
